;;; bili-comment-test.el --- Tests for Bilibili comments  -*- lexical-binding: t; -*-
...
(ert-deftest bili-comment-uses-appkit-discussion-and-cursor-pagination ()
                  ...
                  (funcall
                   ...
                   (if offset
                     ...
                     `((cursor
                       ...
                       (top_replies . (,pinned))
                       (replies . (,root)))))))))
          (setq view (bili-comment-open video)
                buffer (appkit-view-buffer view)
                observer
                (buffer-local-value 'bili-comment--scroll-observer buffer))
          (bili-comment--maybe-auto-load view nil 100 100)
          ...
          (with-current-buffer buffer
            (let* ((root-key '(comment 42 1))
              ...
              (should (equal
                       ...
                       root-key))
              (should (stringp (get-text-property root-start 'line-prefix)))
              (should (eq (char-after root-start) ?A))
              (goto-char root-start)
              (let ((match
                     (text-property-search-forward
                      'display nil
                      (lambda (_value candidate)
                        (and (listp candidate)
                             (eq (car candidate) 'space)
                             (eq (nth 1 candidate) :align-to))))))
                (should match)))))
      (bili-core-stop)
      (when observer
